...
description:
  Muchos dias y algunas promesas han pasado desde la ultima publicación.
  Pero volvemos estudiando el ataque que sufrió Tornado Cash hace algunos dias,
  mientras aprendemos algunas cosas interesantes sobre Solidity y la EVM, como por ejemplo,
  desplegar dos contratos diferentes en la misma dirección. Si quires saber como hacerlo, ¡vamos a verlo!
...
Tornado Cash es (o era) un protocolo financiero descentralizado (DeFi) que permite a los usuarios realizar transacciones anónimas en Ethereum.
...
en partes más pequeñas que luego eran mezcladas con los depositos de los demás usuarios. Una vez finalizada la mezcla, el usuario podía retir sus Ethers anonimizados.
...
tiempo en pasado porque al parecer el protocolo ya no está disponible para nadie. Su página web fue removida y todos sus repositorios de GitHub, archivados.
...
- Retirar todos los token de la governanza.
- Bloquear el Router.
...
Una vez que la propuesta era aprobada por los votantes, el atacante simplemente utilizaba la función `emergencyStop` para actualizar la lógica
de la propuesta y concederse a sí mismo los votos fraudulentos.

Ahora que el atacante tiene todos los votos, pueden hacer lo que quiera. En este caso, simplemente retiró 10.000 votos como TORN y los vendió todos.
...
Se que si llegaste hasta aquí es porque quires saber como se ve toda esta teoría en la practica. ¡Asi que vamos con eso!

El primer lugar el atacante debía desplegar una `proposal` que debía ser revisada y aprovaba por la governanza de Tornado Cash.
...
Imaginemos el mismo proceso que antes, pero esta vez con una diferencia. Antes de create el contrato malicioso, el atacante "borra" el contrato `Deployer` a travez de la función `selfdestruct`.
